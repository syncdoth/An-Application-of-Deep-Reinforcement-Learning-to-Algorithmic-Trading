...
class PerformanceEstimator:
    ...
    def computeSortinoRatio(self, riskFreeRate=0):
        ...
        if expectedReturn != 0 and volatility != 0:
            self.sortinoRatio = np.sqrt(252) * (expectedReturn -
    ...
    def computeProfitability(self):
        ...
        loss = 0
        index = next(
            (i for i in range(len(self.data.index)) if self.data['Action'][i] != 0), None)
        if index == None:
    ...
    def displayPerformance(self, name):
        ...
        tabulation = tabulate(self.performanceTable,
                              headers,
                              tablefmt="fancy_grid",
                              stralign="center")
        print(tabulation)
